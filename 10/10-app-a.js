...
use("POST","todo",function(request,response){
        ...
        if(error){
            ...
            writeFile('./todo.json', dataString, function (error){
                if(error){
                    ...
                    write(response, 200, JSON.stringify({id:id}))
                }
            });
        }
    });
...
use("GET","todo",function(request,response){
    readFile('./todo.json', 'utf8', function (error, fileData){
        if(error){
            console.log('ERROR:', error);
            write(response, 500, 'ERROR:' + error);
...
use("PUT","todo",function(request,response){
        ...
        if(error){
            ...
            writeFile('./todo.json', dataString, function (error){
                if(error){
                    ...
                    console.log('Todo Created.');
                    write(response, 200, "Status Changed")
                }
            });
        }
    });
